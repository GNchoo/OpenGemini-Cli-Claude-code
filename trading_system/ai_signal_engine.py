...
class MeanReversion:
    ...
    def analyze(self, prices: List[float], **kwargs) -> Dict:
        ...
        if rsi < 20:
            ...
            score -= 0.3
        # RSI 60~70은 정상 상승 구간이므로 SELL 제거 (기존 -0.2)
        ...
        if bb_pos < 0.1:  # 하단 돌파
            ...
            score += 0.3
        elif bb_pos > 0.95:  # 극단적 상단 돌파만 (기존 0.9)
            score -= 0.4
        elif bb_pos > 0.85:  # 기존 0.8
            score -= 0.2
    ...
    def _make_signal(self, score: float, indicators: Dict) -> Dict:
        confidence = min(abs(score) * 1.0, 1.0)  # 신뢰도 배수 1.1→1.0 (과신 방지)
        
        if score > 0.4:
            signal = "BUY"
        elif score < -0.5:  # SELL 임계 강화 (기존 -0.4 → -0.5)
            signal = "SELL"
        else:
            signal = "HOLD"
...
class RiskController:
    ...
    def __init__(self):
        ...
        self.weight = 1.0  # 다른 에이전트와 동등 (기존 1.5에서 하향)
...
class AISignalEngine:
    ...
    def decide(self, symbol: str, prices: List[float], volatility: float,
        ...
        # 최소 신뢰도 (프로필 레이어에서 한번 더 체크하므로 여기선 완화)
        min_conf = {
            "AGGRESSIVE": 0.50,
            "MODERATE": 0.55,
            "CONSERVATIVE": 0.60
        ...
        # 상위 프로젝트와 동일: 에이전트 전원 동일 신호일 때만 만장일치
        agent_signals = [r["signal"] for r in results]
        unanimous = len(set(agent_signals)) == 1
